...
export class ERecoredChanged extends Exception
{
    constructor()
    {
        super('e_record_changed');
    }
}
...
/* ISqlEngineSupport */
    ...
    IsInsertOrUpdate(sql: string): boolean
    {
        ...
        if (sql[0] === ' ')
            test = sql.trim().substr(0, 6).toUpperCase();
        else
            test = sql.substr(0, 6).toUpperCase();

        return test === 'INSERT' || test === 'UPDATE';
    }
...
/* ISqlConnectionPool */
    ...
    async Execute<TRetVal>(Callback: (conn: TSqlConnection, ...args: any[]) => Promise<TRetVal>, ...args: any[]): Promise<TRetVal>
    {
        ...
        try
        {
            const RetVal = await Callback(conn, ...args);
...
/* IStorageEngine */
    ...
    SaveObject(Obj: IPersistable): Promise<void>
    {
        return this.Execute<void>(conn => conn.SaveObject(Obj));
    }

    RemoveObject(Obj: IPersistable): Promise<void>
    {
        return this.Execute<void>(conn => conn.RemoveObject(Obj));
    }
}
        ...
        for (const Rule of Rules)
        {
            try
            {
                ...
                for (const str of Rule.KeyProps)
                {
                    ...
                    // but only in update when difference
                    if (Updates.indexOf(str) === -1 && NewValue !== OldValue)
                    {
                        Updates.push(str);
...
export class TSqlStrictQuery extends TSqlQuery
{
    HandleResult(Result: any, EffectRows: number): void
    {
        ...
        if (EffectRows === 0)
        {
            ...
            throw new ERecoredChanged();
        }
        else if (EffectRows !== 1)
        {
            // console.log(this.sql);
            throw new Error('Effect more than 1 record');
            // console.log('Effect more than 1 record');
            // console.log('**Debug here?**, somehow all cols effect count to rows');
        }
    }
}
