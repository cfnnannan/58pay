...
import {InvError, EInvAbort} from './InvError';
...
class TExpressInvApplication extends TExpressApplication
{
    ...
    private ImplementInvMethod(metadata: InvMethodMetadata): this
    {
        this.Instance.all('/' + metadata.Name, (req, res, next) =>
        {
            try
            {
                ...
                default:
                    metadata.InvokeCheck(req.headers, req.method as InvHttp.Method, req.query);
                    ...
                    const RetVal = metadata.Entry.bind(ObjectInstance)(req.query, req.body);

                    /*
                    let ContentType = this.metadata.Options.ContentType;
                    if (ContentType === '')
                        ContentType = InvHttp.ParseRequestContentType(req.headers['content-type'] as string);
    ...
    private async ResolveInvRetVal(RetVal: any, Response: Express.Response, Ctx?: TInvokableClass): Promise<void>
    {
        ...
        {
            if (TypeInfo.IsObject(RetVal))
                RetVal = JSON.stringify(RetVal);
            else if (! TypeInfo.IsString(RetVal))
                RetVal = JSON.stringify(RetVal);
...
export class TExpressRoot extends TExpressApplication
{
    CreateServer(Port: number)
    {
        ...
        const http = Http.createServer(this.Instance as any).listen(Port, () =>
